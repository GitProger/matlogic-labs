...
main = do 
  ...
  let strs = map (parseProof . alexScanTokens) input
    ...
    let res = zipWith3 (\i orig res -> (i, withProofLine orig res)) [1..] strs ans
        inc = V.fromList $ map (\(_, me) -> isIncorrect me) res
        resX = map (\(i, me) -> (i, validate me inc)) res 
      in
        putStr . unlines $ map (\(i, me) -> numbered i $ show me) resX
